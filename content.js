(function() {
  ...
  function updateTooltipContent(content, url) {
    ...
    const shortUrl = url ? getShortUrl(url) : 'unknown';
    const wasShowing = currentlyDisplayedUrl ? getShortUrl(currentlyDisplayedUrl) : 'none';
    if (!currentlyDisplayedUrl || currentlyDisplayedUrl !== url) {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    // ============ VOICE RECOGNITION (DELEGATED TO SIDEPANEL) ============
    if (message.type === 'EXECUTE_VOICE_COMMAND') {
      ...
      if (command === "scroll down") {
        ...
        if (currentHoveredElement) currentHoveredElement.click();
      }
      ...
      sendResponse({ status: 'ok' }); // explicitly close the message channel
      return true;
    }
  });
  ...
  // Get initial display mode, gaze, and voice enabled status
