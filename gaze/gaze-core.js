(function() {
  ...
  const HEAD_FILTER_MIN_CUTOFF = 0.2;     // Reduced to 0.2 for ultra-smooth static holding
  const HEAD_FILTER_BETA = 0.0015;        // Fine-tuned beta for responsive velocity scaling
  const HEAD_FILTER_D_CUTOFF = 1.0;
  const HEAD_POINTER_LERP = 0.08;
...
let headAutoCenter = { nx: 0, ny: 0, ready: false };
  ...
  function computeHeadFrame(face) {
    ...
    let leftEye = (mesh && mesh[33]) ? mesh[33] : (annotations && annotations.leftEyeUpper0 ? annotations.leftEyeUpper0[0] : null);
    let rightEye = (mesh && mesh[263]) ? mesh[263] : (annotations && annotations.rightEyeUpper0 ? annotations.rightEyeUpper0[0] : null);
    let noseTip = (mesh && mesh[1]) ? mesh[1] : (annotations && annotations.noseTip ? annotations.noseTip[0] : null);
  ...
  async function ensureInitialized() {
    ...
    initializationPromise = (async () => {
      dispatchStatus('loading', 'Loading Human.js (WebGL)');
      ...
      try {
        ...
        await human.load();
        await human.warmup();
        console.log('[GazeCore] Human.js engine initialized!');
      } catch (error) {
        console.error('[GazeCore] Failed to initialize Human.js:', error);
        dispatchStatus('ready', 'Human.js initialization failed');
        throw error;
      }
      ...
      if (headCal && headCal.version === 2) {
        dispatchStatus('live', 'Head pointer ready (Human.js)');
      } else {
  ...
  function startDetectionLoop() {
    ...
    const processFrame = async () => {
      ...
      if (!detectInProgress && video.readyState >= 2) {
        detectInProgress = true;
        try {
          const result = await human.detect(video);
          processDetection(result, performance.now());
        } catch (err) {
          console.warn('[GazeCore] Human.js detect error:', err);
        } finally {
          detectInProgress = false;
        }
      }

      if (video && human && gazeEnabled) {
    ...
    if (window.__gazeHeadMode) {
      if (headFrame) {
        ...
        if (mapped) {
          ...
          if (lastHeadPoint) {
            // Distance-adaptive LERP algorithm (ported & enhanced from BUG)
            const dx = finalX - lastHeadPoint[0];
            const dy = finalY - lastHeadPoint[1];
